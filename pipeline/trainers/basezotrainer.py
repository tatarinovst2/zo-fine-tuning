# pylint: skip-file  # Code mostly copied from transformers
"""Module containing a base ZO-Trainer class which implements a custom _inner_training_loop."""
import contextlib
import functools
import os
import shutil
...
from transformers.trainer import _is_peft_model, Trainer, TRAINER_STATE_NAME
from transformers.trainer_callback import (DefaultFlowCallback, ExportableState, ProgressCallback,
                                           TrainerState)
from transformers.trainer_pt_utils import get_model_param_count
from transformers.trainer_utils import speed_metrics, TrainOutput
from transformers.utils import (is_accelerate_available, is_in_notebook, is_sagemaker_mp_enabled,
                                ...
                                is_torch_xpu_available, XLA_FSDPV2_MIN_VERSION)
...
if is_in_notebook():
    ...
    DEFAULT_PROGRESS_CALLBACK = NotebookProgressCallback  # type: ignore
...
class BaseZOTrainer(Trainer):
    """Base ZO class with a custom _inner_training_loop to be customized by children."""

    def __init__(self, **kwargs):
        """
        Initialize the BaseZOTrainer.

        :param kwargs: Keyword arguments forwarded to ``Trainer``.
        """
        super().__init__(**kwargs)

        np.random.seed(self.args.seed)
        self.trainer_mode = getattr(self.args, "trainer_mode", "zo")
    ...
    def zo_perturb_parameters(self, scaling_factor: float = 1.0):
        ...
        :raises NotImplementedError: If the method is not implemented by a descendant class.
        """
        raise NotImplementedError("`zo_perturb_parameters` method must be implemented by a descendant class.")

    def zo_forward(self, model: PreTrainedModel, inputs: dict[str, Any]) -> torch.Tensor:
        """
        Compute the loss.
    ...
    def zo_step(self, model: PreTrainedModel, inputs: dict[str, Any]) -> torch.Tensor:
        ...
        :raises NotImplementedError: If the method is not implemented by a descendant class.
        """
        raise NotImplementedError("`zo_step` method must be implemented by a descendant class.")
    ...
    def zo_update(self, learning_rate: float) -> None:
        ...
        :raises NotImplementedError: If the method is not implemented by a descendant class.
        """
        raise NotImplementedError("`zo_update` method must be implemented by a descendant class.")

    def _maybe_empty_cache(self) -> None:
        """Empty the cache if `torch_empty_cache_steps` is set and we are at the specified step."""
                ...
                for i, inputs in enumerate(batch_samples):
                    ...
                    if do_sync_step:
                        ...
                        ########################
                            ...
                            with context():
                                self.optimizer.step_after_v_refresh()
